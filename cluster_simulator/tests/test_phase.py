...
from cluster_simulator.phase import Pattern, Operation, PhaseFeatures
...
class TestMixedPhase(unittest.TestCase):
    ...
    def test_run_mixed_phase_persistent(self):
    ...
    def test_run_mixed_phase_persistent_2(self):
        """Test running simple readwrite phase on persistent different tier."""
        # define an MixIO phase
        mixed_io_phase = MixIOPhase(read_volume=1e9, write_volume=10e9, data=self.data)
        # define burst buffer with its backend PFS
        cluster = Cluster(self.env, tiers=[self.ssd_tier, self.nvram_tier])
        # run the phase on the tier with placement = bb
        self.env.process(mixed_io_phase.run(self.env, cluster, placement=1, use_bb=False))
        self.env.run()
...
class TestPhaseFeatures(unittest.TestCase):
    ...
    def test_default_phase_features(self):
        ...
        assert pf.read_pattern == Pattern.SEQ
        assert pf.write_pattern == Pattern.SEQ
    ...
    def test_init_phase_features_read_with_pattern(self):
        ...
        assert pf.write_volume == 0
        assert pf.read_pattern == Pattern.RAND
        assert pf.write_pattern == Pattern.SEQ
        assert pf.read_io_size == 4e3
        assert pf.write_io_size == 4e3
    ...
    def test_init_phase_features_write_with_pattern(self):
        ...
        assert pf.read_pattern == Pattern.SEQ
        assert pf.write_pattern == Pattern.RAND
        assert pf.read_io_size == 4e3
        assert pf.write_io_size == 4e3
